...
# Function to generate non-streaming response
def generate_response(prompt, model_name, max_tokens, temperature, chat_history=[]):
    try:
        if not prompt.strip():
            return chat_history, "Please enter a valid prompt."

        client = load_model(model_name)
        if isinstance(client, str):
            return chat_history, client
        ...
        return updated_history, "Response generated."
    except Exception as e:
...
def create_gradio_interface():
    with gr.Blocks(theme=gr.themes.Soft()) as demo:
        gr.Markdown("# GenAI Text Generation App (OpenAI, Non-Streaming)")
